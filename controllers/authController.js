...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    return next(new AppError('this account no longer exist', 401));
  }
...
exports.restrictTo = (...roles) => {
  return (req, res, next) => {
    if (!roles.includes(req.user.role)) {
      return next(
        new AppError('you dont have the permission to do this !', 401)
      );
    }
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError('there is no account with this email address !', 401)
    );
  }
